...
/* ── Auto-cycling carousel (same fade+scale as homepage cards) ──────────────── */
    ...
    >
      ...
      <div className="relative h-72 overflow-hidden">
        ...
        <div className="absolute top-4 left-4">
          <span className="font-mono text-[10px] tracking-[.14em] uppercase px-3 py-1.5 rounded-full border"
            style={{ color: ACCENT, borderColor: `${ACCENT}50`, background: 'rgba(10,6,20,0.72)', backdropFilter: 'blur(8px)' }}>
            {tag}
          </span>
        </div>
        {/* Room number */}
      ...
      <div className="p-7">
        ...
        <div className="flex flex-wrap gap-2 pt-4 border-t border-black/6">
          {specs.map((s, i) => (
            <span key={i} className="font-mono text-[9.5px] tracking-[.12em] uppercase px-2.5 py-1.5 rounded-full bg-[#f3f0ff] text-[#7c3aed]">
              {s}
            </span>
          ))}
        </div>
...
function FacilityCard({ img, label, desc, delay = 0 }) {
  return (
    ...
    >
      ...
      <div className="absolute inset-0 bg-linear-to-t from-[#0d0a18]/80 via-[#0d0a18]/20 to-transparent" />
...
/* ── Main page ──────────────────────────────────────────────────────────────── */
export default function LuxClinicPage() {
  ...
  return (
    <main className="min-h-screen overflow-hidden" style={{ background: '#faf9f7' }}>
      ...
      {/* ══════════════════════════════════════════════════════════════
          01 · HERO
      ══════════════════════════════════════════════════════════════ */}
      <section className="relative min-h-screen flex items-end overflow-hidden" style={{ background: '#0d0a18' }}>
        {/* Ambient glow */}
        <div className="absolute inset-0 pointer-events-none"
          style={{ background: `radial-gradient(ellipse 65% 55% at 20% 30%, ${ACCENT}18, transparent 60%)` }} />
        <div className="absolute inset-0 pointer-events-none"
          style={{ background: 'radial-gradient(ellipse 50% 40% at 80% 70%, rgba(167,139,250,0.08), transparent 55%)' }} />
        ...
        <div className="absolute top-0 right-0 bottom-0 w-[52%] max-lg:w-full max-lg:opacity-20 grid grid-rows-2">
          <div className="relative overflow-hidden">
            <motion.img src={luxReception} alt="Lux Reception"
              className="w-full h-full object-cover object-center"
              initial={{ scale: 1.08 }} animate={{ scale: 1 }}
              transition={{ duration: 1.8, ease: [0.2, 0.8, 0.2, 1] }} />
            <div className="absolute inset-0 bg-linear-to-l from-transparent via-[#0d0a18]/30 to-[#0d0a18]" />
          </div>
          <div className="relative overflow-hidden border-t border-white/8">
            <motion.img src={luxReception2} alt="Lux Reception"
              className="w-full h-full object-cover object-center"
              initial={{ scale: 1.08 }} animate={{ scale: 1 }}
          ...
          <motion.div variants={stagger(0.1)} initial="hidden" animate="visible">
            ...
            <motion.div variants={fadeUp} transition={t(0.7)} className="flex flex-wrap gap-3">
              ...
              <button onClick={() => setBookOpen(true)}
                ...
                style={{ background: '#fff', color: '#0d0a18', border: `1px solid ${ACCENT}33` }}>
      ...
      {/* ══════════════════════════════════════════════════════════════
          02 · STATS
      ══════════════════════════════════════════════════════════════ */}
      <section style={{ background: '#13101e' }} className="border-b border-white/5">
        <div className="max-w-360 mx-auto px-10 max-sm:px-6 py-9">
          <div className="grid grid-cols-4 max-sm:grid-cols-2 gap-8">
            {[
              { val: '12+', label: 'Private Suites' },
              { val: '6',   label: 'Specialties'    },
              { val: 'VIP', label: 'Membership'     },
              { val: '5★',  label: 'Experience'     },
            ].map((s, i) => (
              <motion.div key={i}
      ...
      <section className="py-28 px-10 max-sm:px-6 max-sm:py-20" style={{ background: '#faf9f7' }}>
        <div className="max-w-360 mx-auto">
          <motion.div variants={stagger(0.08)} initial="hidden" whileInView="visible" viewport={viewport}
            ...
            <div className="grid lg:grid-cols-[1fr_1.2fr] gap-12 items-end">
              <motion.h2 variants={fadeUp} transition={t()}
                className="font-serif font-light leading-[1.0] tracking-[-0.025em] text-[#1a1025]"
                style={{ fontSize: 'clamp(36px,5vw,62px)' }}>
                Every room,<br /><span className="italic" style={{ color: ACCENT }}>a statement.</span>
              </motion.h2>
              <motion.p variants={fadeUp} transition={t(0.7)}
          ...
          <div className="grid lg:grid-cols-2 gap-8">
            <SuiteCard num="01" name="GP Consultation Suite"
              tag="General Practice · 200 sqft"
              desc="A calming consultation sanctuary designed for comprehensive general practice. Organic wall art, warm ambient lighting and a dedicated consultation zone built for clarity and comfort."
              imgs={[luxGp1, luxGp2, luxGp3]}
              specs={['Hi-Lo Exam Table', 'EMR Ready', 'DHA Licensed', 'Private']}
              delay={0} />
            <SuiteCard num="02" name="Dental Suite"
              tag="Dentistry · 220 sqft"
              desc="A premium dental operatory with signature tooth-chart wall art, surgical-grade overhead lighting and state-of-the-art equipment set within a serene, anxiety-free environment."
              imgs={[luxDental1, luxDental2]}
              specs={['Full Chair Unit', 'Intraoral Imaging', 'Class-B Autoclave', 'RVG Ready']}
              delay={0.1} />
            <SuiteCard num="03" name="Dermatology Room"
              tag="Dermatology · 180 sqft"
              desc="A skin-first environment with 5500K full-spectrum lighting, face line-art walls and a fully adjustable procedure bed, built for flawless clinical and aesthetic dermatology."
              imgs={[luxDerma1, luxDerma2]}
              specs={['5500K Lighting', 'CRI 95+', 'Dermatoscope', 'Procedure Ready']}
              delay={0.2} />
            <SuiteCard num="04" name="Treatment Suite"
              tag="Treatment · 240 sqft"
              desc="A signature treatment suite with arched mirror, warm timber accents and a fully articulated treatment bed. Used for aesthetics, IV therapy and minor procedures."
              imgs={[luxTreatment1, luxTreatment2, luxTreatment3]}
              specs={['Articulated Bed', 'IV Ready', 'Warm Lighting', 'Sterile Zone']}
              delay={0.3} />
          </div>
        </div>
      </section>
      ...
      {/* ══════════════════════════════════════════════════════════════
          04 · LASER & THERAPY — full-bleed split
      ══════════════════════════════════════════════════════════════ */}
      <section className="grid lg:grid-cols-2 grid-cols-1 min-h-[600px] max-lg:min-h-0">
        {/* Laser Room */}
        <div className="relative min-h-80 lg:min-h-full overflow-hidden group">
          <motion.img src={luxLaser} alt="Laser Room"
            className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105"
            initial={{ scale: 1.06 }}
            whileInView={{ scale: 1 }}
            viewport={{ once: true }}
            transition={{ duration: 1.2, ease: [0.2, 0.8, 0.2, 1] }} />
          <div className="absolute inset-0 bg-linear-to-t from-[#0d0a18]/75 via-[#0d0a18]/20 to-transparent" />
          <div className="absolute bottom-0 left-0 p-10 max-sm:p-6">
            <p className="font-mono text-[10px] tracking-[.18em] uppercase mb-2" style={{ color: ACCENT }}>05 · Laser Room</p>
            <h3 className="font-serif text-[32px] font-light text-white leading-[1.1]">
              Precision<br /><em style={{ color: ACCENT }}>laser therapy.</em>
            </h3>
            <p className="text-white/50 text-[13.5px] mt-3 max-w-[300px] leading-[1.65]">
              Dedicated laser suite with safety-compliant clearances, premium device bays and a serene recovery zone.
            </p>
          </div>
        </div>
        ...
        {/* Therapy Room */}
        <div className="relative min-h-80 lg:min-h-full overflow-hidden group">
          <motion.img src={luxTherapy} alt="Therapy Room"
            className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105"
            initial={{ scale: 1.06 }}
            whileInView={{ scale: 1 }}
            viewport={{ once: true }}
            transition={{ duration: 1.2, delay: 0.1, ease: [0.2, 0.8, 0.2, 1] }} />
          <div className="absolute inset-0 bg-linear-to-t from-[#0d0a18]/80 via-[#0d0a18]/25 to-transparent" />
          <div className="absolute bottom-0 left-0 p-10 max-sm:p-6">
            <p className="font-mono text-[10px] tracking-[.18em] uppercase mb-2" style={{ color: ACCENT }}>06 · Therapy Room</p>
            <h3 className="font-serif text-[32px] font-light text-white leading-[1.1]">
              Immersive<br /><em style={{ color: ACCENT }}>healing space.</em>
            </h3>
            <p className="text-white/50 text-[13.5px] mt-3 max-w-[300px] leading-[1.65]">
              A cocoon of warmth, timber slat walls, ambient LED arches and a purpose-built therapy plinth.
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="py-28 px-10 max-sm:px-6 max-sm:py-20" style={{ background: '#fff' }}>
        <div className="max-w-360 mx-auto">
          <motion.div variants={stagger(0.08)} initial="hidden" whileInView="visible" viewport={viewport}
            ...
            <motion.h2 variants={fadeUp} transition={t()}
              className="font-serif font-light tracking-[-0.025em] text-[#1a1025]"
          ...
          <div className="grid grid-cols-12 grid-rows-[auto] gap-4">
            ...
            {/* Row 1 — large + small */}
            <motion.div className="col-span-8 max-sm:col-span-12 row-span-2 rounded-3xl overflow-hidden h-[480px] max-sm:h-64"
              initial={{ opacity: 0, scale: 0.97 }}
              whileInView={{ opacity: 1, scale: 1 }}
              viewport={viewport}
              transition={{ duration: 0.75, ease: snap }}>
              <Carousel imgs={[luxDerma2, luxDerma1]} className="relative w-full h-full" objectPos="center top" />
            </motion.div>
            ...
            <motion.div className="col-span-4 max-sm:col-span-12 rounded-3xl overflow-hidden h-56 max-sm:h-48"
              ...
              viewport={viewport}
              transition={{ duration: 0.65, delay: 0.1, ease: snap }}>
              <img src={luxVanity} alt="Female Vanity" className="w-full h-full object-cover" />
            </motion.div>
            ...
            <motion.div className="col-span-4 max-sm:col-span-12 rounded-3xl overflow-hidden h-56 max-sm:h-48 relative"
              ...
              whileInView={{ opacity: 1, x: 0 }}
              viewport={viewport}
              transition={{ duration: 0.65, delay: 0.2, ease: snap }}>
              <img src={luxSterilize} alt="Sterilization" className="w-full h-full object-cover" />
              <div className="absolute inset-0 bg-linear-to-t from-[#0d0a18]/60 to-transparent" />
              <div className="absolute bottom-4 left-4">
                <p className="font-mono text-[9px] tracking-[.14em] uppercase text-white/60">Clinical Grade</p>
                <p className="font-serif text-[16px] font-light text-white">Sterilization</p>
              </div>
            </motion.div>
            ...
            <motion.div className="col-span-4 max-sm:col-span-12 rounded-3xl overflow-hidden h-56 max-sm:h-48"
              ...
              whileInView={{ opacity: 1, y: 0 }}
              viewport={viewport}
              transition={{ duration: 0.65, delay: 0.05, ease: snap }}>
              <img src={luxGp2} alt="GP Room" className="w-full h-full object-cover" />
            </motion.div>
            ...
            <motion.div className="col-span-4 max-sm:col-span-12 rounded-3xl overflow-hidden h-56 max-sm:h-48"
              ...
              whileInView={{ opacity: 1, y: 0 }}
              viewport={viewport}
              transition={{ duration: 0.65, delay: 0.12, ease: snap }}>
              <img src={luxTreatment3} alt="Treatment Room" className="w-full h-full object-cover" />
            </motion.div>
            ...
            <motion.div className="col-span-4 max-sm:col-span-12 rounded-3xl overflow-hidden h-56 max-sm:h-48"
              ...
              whileInView={{ opacity: 1, y: 0 }}
              viewport={viewport}
              transition={{ duration: 0.65, delay: 0.19, ease: snap }}>
              <img src={luxDental2} alt="Dental Room" className="w-full h-full object-cover" />
            </motion.div>
            ...
            <motion.div className="col-span-5 max-sm:col-span-12 rounded-3xl overflow-hidden h-64 max-sm:h-48"
              ...
              whileInView={{ opacity: 1, y: 0 }}
              viewport={viewport}
              transition={{ duration: 0.65, delay: 0.08, ease: snap }}>
              <img src={luxGp3} alt="GP Room" className="w-full h-full object-cover object-top" />
            </motion.div>
            ...
            <motion.div className="col-span-4 max-sm:col-span-12 rounded-3xl overflow-hidden h-64 max-sm:h-48"
              ...
              whileInView={{ opacity: 1, y: 0 }}
              viewport={viewport}
              transition={{ duration: 0.65, delay: 0.14, ease: snap }}>
              <img src={luxTreatment2} alt="Treatment" className="w-full h-full object-cover" />
            </motion.div>
            ...
            <motion.div className="col-span-3 max-sm:col-span-12 rounded-3xl overflow-hidden h-64 max-sm:h-48 relative"
              ...
              viewport={viewport}
              transition={{ duration: 0.65, delay: 0.2, ease: snap }}>
              <img src={luxPantry} alt="Pantry" className="w-full h-full object-cover" />
              <div className="absolute inset-0 bg-linear-to-t from-[#0d0a18]/65 to-transparent" />
              <div className="absolute bottom-4 left-4">
                <p className="font-mono text-[9px] tracking-[.14em] uppercase text-white/55">Wellness</p>
                <p className="font-serif text-[15px] font-light text-white">Pantry</p>
              </div>
            </motion.div>
          </div>
        </div>
      ...
      {/* ══════════════════════════════════════════════════════════════
          06 · FACILITIES GRID
      ══════════════════════════════════════════════════════════════ */}
      <section className="py-24 px-10 max-sm:px-6 max-sm:py-16" style={{ background: '#0d0a18' }}>
        <div className="max-w-360 mx-auto">
          <motion.div variants={stagger(0.08)} initial="hidden" whileInView="visible" viewport={viewport}
            className="mb-12">
            <motion.div variants={fadeUp} transition={t(0.6)} className="flex items-center gap-4 mb-5">
          ...
          <div className="grid lg:grid-cols-4 sm:grid-cols-2 grid-cols-2 gap-4">
            <FacilityCard img={luxVanity}    label="Female Vanity"    desc="Premium vanity suite"    delay={0}    />
            <FacilityCard img={luxSterilize} label="Sterilization"    desc="Class-B autoclave lab"  delay={0.08} />
            <FacilityCard img={luxPantry}    label="Wellness Pantry"  desc="Staff & patient refresh" delay={0.16} />
            <FacilityCard img={luxHandicap}  label="Accessibility"    desc="Fully accessible WC"    delay={0.24} />
          </div>
        </div>
      </section>
      ...
      <section className="relative min-h-[55vh] flex items-center overflow-hidden">
        <div className="absolute inset-0">
          <img src={luxTherapy} alt="" className="w-full h-full object-cover" />
          <div className="absolute inset-0" style={{ background: 'rgba(13,10,24,0.78)' }} />
        </div>
        <div className="relative z-10 max-w-360 mx-auto px-10 max-sm:px-6 py-24">
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            ...
            <motion.div
              ...
              className="space-y-5">
              {[
                ['Discreet private entrance & exit', 'No shared waiting, ever.'],
                ['Member concierge programme', 'Dedicated care coordinators.'],
                ['Curated ambient environment', 'Lighting, scent, acoustic design.'],
                ['Premium aftercare lounge', 'Recovery in comfort and privacy.'],
              ].map(([title, sub], i) => (
              ...
              <div>
                {submitted ? (
                  <motion.div
                    ...
                    <p className="text-white/45 text-[13px]">We'll be in touch before launch.</p>
                  </motion.div>
                ) : (
                  <form onSubmit={handleRegister} className="flex flex-col gap-3.5">
                    <input
                      ...
                      className="w-full px-5 py-4 rounded-2xl bg-white/6 border border-white/12 text-white placeholder:text-white/30 text-[14px] outline-none focus:border-purple-400/50 transition-colors"
                      required
                    />
                    <button type="submit"
                      className="w-full py-4 rounded-2xl text-[14px] font-medium transition-all hover:-translate-y-px"
                      style={{ background: ACCENT, color: '#0d0a18' }}>
      ...
      {/* Booking modal (Lux theme override) */}
      <div style={{ ['--color-brand']: ACCENT, ['--color-gold']: '#7c3aed' }}>
